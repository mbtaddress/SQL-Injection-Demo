...
if ( isset( $_GET['first_name'], $_GET['last_name'], $_GET['email'], $_GET['password']) ) {
	...
	$update_query = "UPDATE user SET firstname=:first_name, lastname=:last_name, email=:email, phone=:phone, password=:password WHERE id=:id";
	...
	// SAFE: use prepared statement for the fetch-phase query too
	$fetch_query = "SELECT id, firstname, lastname, email, phone FROM user WHERE id = :id";
	$fetch_stmt  = $pdo->prepare($fetch_query);
	$fetch_stmt->execute([':id' => (int) $_GET['id']]);
